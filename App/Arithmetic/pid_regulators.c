...
s32 PIDRegulatorS32_Pos(s32 aim, s32 cur, PIDObjTyp *ppid)
{
	...
	out_value  = (s64)ppid->kp * err;                       //比例
	out_value += (s64)ppid->ki * ppid->ki_plus;          //积分
    out_value += (s64)ppid->kd * (err - ppid->lasterr[0]);  //微分
	...
	// 积分饱和处理
	if(ppid->ki_plus > ppid->max_ki_plus)
		ppid->ki_plus = ppid->max_ki_plus;
	else if(ppid->ki_plus < -ppid->max_ki_plus)
		ppid->ki_plus = -ppid->max_ki_plus;
...
{
	ppid->lasterr[0] = ppid->lasterr[1] = 0;
    ppid->ki_plus = 0;
	ppid->remainder = 0;	
}
